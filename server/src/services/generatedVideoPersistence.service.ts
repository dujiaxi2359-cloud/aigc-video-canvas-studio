  ...
  const nextNodes = nodes.map((node) => {
    ...
    return {
      ...
      data: {
        ...
        providerVideoUrl: input.downloadableUrl ?? input.outputUrl,
        providerTaskId: input.providerTaskId ?? data.providerTaskId,
        cdnUrl: input.cdnUrl ?? data.cdnUrl,
        cosUrl: input.cosUrl ?? data.cosUrl,
        posterUrl: input.posterUrl ?? data.posterUrl,
        previewUrl: input.previewUrl ?? input.outputUrl,
        thumbnailUrl: input.thumbnailUrl ?? data.thumbnailUrl,
        downloadUrl: input.downloadableUrl ?? input.outputUrl,
