...
export function PlasmaWave(props: PlasmaWaveProps) {
  ...
  useEffect(() => {
    ...
    const colorOne = colors[0] ?? "#A855F7";
    const colorTwo = colors[1] ?? "#06B6D4";
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uBend1: { value: bend1 },
        uBend2: { value: bend2 },
        uColor1: { value: hexToRgb(colorOne) },
        uColor2: { value: hexToRgb(colorTwo) }
      }
    });
    ...
    const update = (now: number) => {
      const {
        xOffset: nextXOffset = 0,
        yOffset: nextYOffset = 0,
        rotationDeg: nextRotation = 0,
        focalLength: nextFocalLength = 0.8,
        speed1: nextSpeed1 = 0.05,
        speed2: nextSpeed2 = 0.05,
        dir2: nextDir2 = 1,
        bend1: nextBend1 = 1,
        bend2: nextBend2 = 0.5,
        colors: nextColors = ["#A855F7", "#06B6D4"]
      } = propsRef.current;

      uniformOffset[0] = nextXOffset;
      uniformOffset[1] = nextYOffset;
      program.uniforms.iTime.value = (now - startTime) * 0.001;
      program.uniforms.uRotation.value = (nextRotation * Math.PI) / 180;
      program.uniforms.uFocalLength.value = nextFocalLength;
      program.uniforms.uSpeed1.value = nextSpeed1;
      program.uniforms.uSpeed2.value = nextSpeed2;
      program.uniforms.uDir2.value = nextDir2;
      program.uniforms.uBend1.value = nextBend1;
      program.uniforms.uBend2.value = nextBend2;
      program.uniforms.uColor1.value = hexToRgb(nextColors[0] ?? "#A855F7");
      program.uniforms.uColor2.value = hexToRgb(nextColors[1] ?? "#06B6D4");
  ...
  return <div ref={containerRef} className={["plasma-wave-container", className].filter(Boolean).join(" ")} />;
}
