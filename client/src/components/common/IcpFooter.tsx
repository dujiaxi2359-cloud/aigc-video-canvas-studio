...
export function IcpFooter({ className = "" }: { className?: string }) {
  return (
    <footer className={`pointer-events-none fixed inset-x-0 bottom-0 z-50 px-5 pb-4 ${className}`}>
      <div className="mx-auto flex max-w-[1240px] flex-col items-center justify-center gap-1 border-t border-white/[0.06] pt-3 text-center text-[11px] leading-5 text-white/34">
        <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
          {LEGAL_LINKS.map((link) => (
            <a key={link.href} href={link.href} className="pointer-events-auto transition hover:text-white/70">
              {link.label}
            </a>
          ))}
        </div>
